...
**Intento de recorte digital, revertido.** Con la cámara bien enfocada pero el código pequeño en el encuadre en el mismo Fire tablet, se probó a recortar y ampliar por software la zona central del fotograma antes de decodificar, para no depender del zoom del hardware (que ya se había descartado por emborronar). Se retiró: el recorte daba por hecho que el vídeo en bruto y el recuadro visual en pantalla comparten proporción, y no es así — el CSS usa `object-fit:cover`, que recorta los lados del vídeo real para encajarlo en el hueco, en una proporción que varía según la resolución que conceda la cámara de cada dispositivo. Calcularlo mal dejaba el código fuera de la zona analizada, y **rompió el escaneo en un iPhone donde antes sí funcionaba** — confirmado por el usuario. Se volvió al enfoque simple (decodificar el fotograma completo, sin recortes), que es el que está en producción. El código de barras pequeño en el encuadre en el Fire tablet concreto donde se detectó queda sin resolver; ver `PENDIENTES.md`.
