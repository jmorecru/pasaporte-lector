...
**Detalle al tocar una insignia**: cada una explica de qué trata al pulsarla — descripción, progreso actual ("llevas 3 de 5 libros") si aún no está lograda, o fecha de consecución si ya lo está. Se decidió deliberadamente **tocar para abrir, no un tooltip al pasar el ratón por encima**: la app se usa sobre todo desde tablets y móviles, donde no existe "pasar por encima", así que un tooltip no lo vería nadie en esos dispositivos. Cada insignia del catálogo lleva además `meta`, `unidad` y `valor(ctx)` (aparte de `check`) solo para poder construir ese texto de progreso.
...
**Intento de recorte digital, revertido.** Con la cámara bien enfocada pero el código pequeño en el encuadre en el mismo Fire tablet, se probó a recortar y ampliar por software la zona central del fotograma antes de decodificar, para no depender del zoom del hardware (que ya se había descartado por emborronar). Se retiró: el recorte daba por hecho que el vídeo en bruto y el recuadro visual en pantalla comparten proporción, y no es así — el CSS usa `object-fit:cover`, que recorta los lados del vídeo real para encajarlo en el hueco, en una proporción que varía según la resolución que conceda la cámara de cada dispositivo. Calcularlo mal dejaba el código fuera de la zona analizada, y **rompió el escaneo en un iPhone donde antes sí funcionaba** — confirmado por el usuario. Se volvió al enfoque simple (decodificar el fotograma completo, sin recortes), que es el que está en producción.

**Cierre del caso del Fire tablet**: falla igual en Silk y en Firefox sobre el mismo dispositivo. Dos motores de renderizado distintos con el mismo fallo apunta al hardware de la cámara, no al navegador ni al código — coincide con que los números impresos del ISBN se ven casi ilegibles a simple vista en esa cámara concreta. Se da por cerrado sin seguir intentando arreglarlo por software; el ISBN tecleado es la vía normal ahí. Estado final por dispositivo: Android ✅, iPhone ✅, Fire tablet ❌ (límite de hardware, no de software).
